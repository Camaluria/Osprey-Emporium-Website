<?php
	//creates products objects to store information from products database, add products, or remove products
	...
	Class Product extends Database {
        ...
        public function get_all_products(){
			...
			$sql= 'SELECT id, item_name, item_description, price, quantity, image_link FROM Products';
		...
		$conn = new mysqli("localhost", $this->username, $this->password, $this->dbname);	 
		$name = trim($name);           
		$sql= "SELECT item_name, item_description, price, quantity, image_link FROM Products Where item_name ='".$name."'";
		$result = $conn->query($sql);
		$rows = [];        
		if ($result->num_rows > 0) {
		while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
		...
		$conn = new mysqli("localhost", $this->username, $this->password, $this->dbname);	             
		$sql= 'SELECT Products.id, item_name, item_description, price, quantity, image_link FROM Products
